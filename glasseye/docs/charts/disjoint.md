...
A disjoint chart is a variety of [force directed graph](force that
includes both attraction and repulsion of disjoint (in the graph
theory sense) graphs. The data given to the disjoint graph is a list 
of *nodes* and a list of *links* between these nodes. The fields of
the node include:

| Field | Type | Require | Description |
| - | - | - | - |
| `id` | String | yes | Unique ID among all nodes |
| `group` | String | yes | Connected group to which node belongs |
| `radius` | Integer | no | Radius of the node's circle, default 1 |

The fields of *links* include:
| Field | Type | Require | Description |
| - | - | - | - |
| Source | ID | yes | The node from which the link *starts* |
| Target | ID | yes | The node at which the link *ends* |
| Value | number | | The size of the link. Defaults to 1 |

Disjoint graphs are invoked as following in html:
...
Note that in this example, with data from the
[D3 example collection](https://observablehq.com/@d3/disjoint-force-directed-graph/2),
extra fields in the nodes (or links) are gracefully ignored.

These both produce the following:

<span id="disjoint_0"></span>
...
<script>
  ...
  "links": [
    ...
    Glasseye.disjoint('#disjoint_0',data,{"width":1000,"height":1000},{},['#A1C9F4', '#FFB482', '#8DE5A1', '#FF9F9B', '#D0BBFF', '#DEBB9B', '#FAB0E4', '#CFCFCF', '#FFFEA3', '#B9F2F0']);
  })
})
</script>
